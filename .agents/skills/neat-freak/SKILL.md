---
name: neat-freak
description: "开发收尾或知识交接时，核对文档、规则与实际实现，并整理本轮工作残留。"
...
| 文档 | 人和下游看到的是不是现役答案？ | README、架构、接入、运维文档、CONTEXT.md 术语表、docs/adr/ 决策记录 |
...
2. **对齐事实**：核对文档说法与代码现状——启动命令、端口、依赖、已实现功能。对不上的，以当前代码为准就地改写；无法当场验证的结论标 `pending`，不写进权威文档。项目有 CONTEXT.md 术语表或 docs/adr/ 时，一并核对术语口径与决策状态是否与现役需求/代码一致，冲突就地修正。
...
- **长期资产一致性**：CONTEXT.md 术语表与现役需求文档 A3/术语章节是否同口径、无过期条目；docs/adr/ 是否有应记未记的架构决定，或被实现/代码已推翻但仍未标注"已替换"的条目；发现冲突 → 就地修正，无法当场裁决的标 `pending` 列入汇报。
...
- [ ] CONTEXT.md 术语与现役需求同口径、无过期条目；docs/adr/ 无应记未记或被推翻未标注的决策。
