...
> 状态：地块职责拆分已完成；本文记录拆分时基线。2026-08-15 又完成一轮按业务域拆分（台风/天气/降水/地块抽为四个 `useXxxMode` composable，`MapView.vue` 收窄为地图核心+装配根，2,105→912 行），架构决策见 `docs/adr/0005-MapView域拆分composable.md`，当前规模以源码为准。
>
> 当时实施结果：`MapView.vue` 从 2,049 行 / 79.8 KB 降至 996 行 / 38.9 KB；纯状态测试从 13 项增至 28 项。当前规模与全仓测试数以源码和最新专项验收文档为准。
>
> 目标文件：`web/src/components/MapView.vue`
>
> 原则：只做结构重构，不改变任何用户可见行为或空间数据口径；本机存储格式以当前 `manualParcelStorage.ts` 为准，允许向后兼容增加人工地块展示序号字段。
...
归档说明：本计划已完成，仅用于追溯历史职责边界和回归清单；当前架构以源码为准，不要把当时测试数或 996 行视为当前项目指标。台风 V1 阶段性浏览器证据见 `docs/archive/灾害风险与台风验收记录.md`。
