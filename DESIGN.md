---
name: 农保云AI智能风控管理平台
description: 面向空间核查、台风态势与点位天气的紧凑工作台。
colors:
  action-blue: "#2563eb"
...
# Design System: 农保云AI智能风控管理平台

## Overview

**Creative North Star: "清晰的空间工具"**

这是一个以地图为主角的空间数据工作台。空间影像、行政边界、地块几何与台风路径是视觉主体；界面只以轻量、半透明的控制浮层停靠在地图边缘，不抢占地图的注意力。信息密度紧凑，但每个动作、状态与反馈都必须能快速扫读。
...
登录是进入工作台前的**独立品牌门**（`web/src/components/LoginView.vue`），与上方地图操作面刻意区分，不继承工作台的蓝/青/紫语义色。

- **背景**：全屏遥感影像（用仓库已提交的 dataProduct 图），叠加深靛蓝→透明渐变**从左向右**贯穿，渐隐透出影像；左实靛蓝为最终方向。
- **布局**：左右分屏，左品牌区约 60%、右登录窗约 40%，左右边距一致（10vw）。
- **左品牌区**：logo 与产品名同一排 lockup（`农保云AI智能风控管理平台`）+ slogan「看清每一个异常，管好每一分经营」+ 3 行能力清单（图标+标签+说明）+ 底部 hairline。能力清单为 `4字·6字` 工整结构：高分遥感=空天观测·灾情空间监测／AI风控=灾害指标·多元智能研判／智能协同=异常即派·线上线下联动。
- **右登录卡**：`.login-card`（auth.spec.ts e2e 依赖）标题「登录系统」，字段「用户名／密码」，页脚版权紧贴卡下方。
- **命名与权限**：产品名已由「农保云AI风险管理平台」改为「农保云AI智能风控管理平台」，并移除品牌层「内部技术验证」；背景素材（吉林一号影像/AI 地块）不得公开分发。
