...
// 极小 ndvi 栅格（3 个点，2 期）
const ndviData = {
  originLon: 120.0, originLat: 29.0, stepLon: 0.005, stepLat: 0.005, cols: 12, rows: 12,
  dates: ['2026-06-01', '2026-07-27'],
  // 12×12=144 格/期，NDVI×100（0=无数据）；混合 5 级让热力图/图例有内容
  layers: [
    Array.from({ length: 144 }, (_, i) => [45, 48, 50, 55, 58, 60, 62, 65, 68, 70, 72, 75][i % 12]),
    Array.from({ length: 144 }, (_, i) => [0, 50, 55, 0, 62, 65, 68, 0, 72, 75, 80, 85][i % 12]),
  ],
}
