...
test('降雨量共用面板风险 tab：统计/列表/下钻详情/村级收起/退出清除', async ({ page }) => {
  ...
  await expect(page.locator('.stat-card.high')).toContainText('高风险 13 村')
  await expect(page.locator('.stat-card.mid')).toContainText('中风险 0 村')
  await expect(page.locator('.stat-card.high .stat-metric strong').first()).not.toBeEmpty()
  await expect(page.locator('.stat-card.high')).toContainText('亩参保面积')
  await expect(page.locator('.stat-card.high')).toContainText('保额')
  await expect(page.locator('.stat-card.high')).toContainText('户')
  ...
  await expect(page.locator('.village-risk-card')).toContainText('风险依据')
  await expect(page.locator('.village-risk-card')).toContainText('保单概况')
  // 7 天降水趋势常驻显示：柱高不溢出（height ≤ 100%），峰值日高亮
  await expect(page.locator('.trend-fill').first()).toBeVisible()
  await expect(page.locator('.village-risk-card')).toContainText('7 天降水趋势')
  await expect(page.locator('.trend-toggle')).toHaveCount(0)
