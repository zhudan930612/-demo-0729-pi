...
test('进入和退出天气保持地图视角，并恢复入口焦点', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '浙江省气象预警', exact: true })).toBeVisible()
  await page.getByRole('button', { name: '关闭浙江预警' }).click()
  await expect(page.getByRole('heading', { name: '浙江省气象预警', exact: true })).toBeHidden()
...
test('气象预警与实时天气模块互斥，实时天气保留完整位置浮窗', async ({ page }) => {
  ...
  await expect(page.getByText('浙江：1 条')).toBeVisible()
  ...
  await expect(alert).toContainText('官方预警正文')
  await expect(alert).toContainText('预计未来三小时部分地区有强降雨')
  await marker.dispatchEvent('mouseleave')
