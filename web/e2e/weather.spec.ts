...
test('进入和退出天气保持地图视角：省级展示 11 个市级标牌并恢复入口焦点', async ({ page }) => {
  ...
  // 并存: 关闭预警后实时天气仍激活, 焦点回到天气按钮(按钮文本为当前模块)
  await expect(page.getByRole('button', { name: '当前：实时天气，点击菜单项可退出' })).toBeFocused()
  expect(await page.locator('.map-zoom-level').textContent()).toBe(before)
})
...
test('点击市级标牌打开完整详情：不导航、浮窗跟随、空白与 Esc 关闭', async ({ page }) => {
  ...
  expect(await page.locator('.map-zoom-level').textContent()).toBe(before)
  await expect(page.getByRole('button', { name: '当前：实时天气，点击菜单项可退出' })).toBeVisible()
  await expect(seatMarkers).toHaveCount(11)
...
test('气象预警与实时天气模块并存且可独立退出', async ({ page }) => {
  await installFixtures(page)
  await openWeather(page)
  ...
  // 两模块并存时按钮提示聚合两个模块
  await expect(page.getByRole('button', { name: '当前：实时天气、气象预警，点击菜单项可退出' })).toBeVisible()
  // 预警面板是右上模态浮层, 遮挡右下工具按钮; 预警模块退出走面板关闭按钮, 实时天气保持
  await page.getByRole('button', { name: '关闭浙江预警' }).click()
  await expect(page.getByRole('heading', { name: '气象预警', exact: true })).toBeHidden()
  await expect(page.locator('.weather-marker-wrap.seat')).toHaveCount(11)
  await expect(popup).toBeVisible()
  await expect(page.getByRole('button', { name: '当前：实时天气，点击菜单项可退出' })).toBeVisible()
  // 菜单项退出实时天气: 点击天气按钮 -> 点击已激活的"实时天气"菜单项
  await page.getByRole('button', { name: '当前：实时天气，点击菜单项可退出' }).click()
  await page.getByRole('button', { name: '实时天气', exact: true }).click()
  await expect(page.locator('.weather-marker-wrap.seat')).toHaveCount(0)
  await expect(page.getByRole('button', { name: '查看天气' })).toBeVisible()
  await expect(popup).toBeHidden()
})
