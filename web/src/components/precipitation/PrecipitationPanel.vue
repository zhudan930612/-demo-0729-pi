<template>
  <aside class="precip-panel" aria-labelledby="precip-panel-title" role="region">
    <header class="panel-header">
      <div class="panel-title"><span class="title-mark" aria-hidden="true"></span><h2 id="precip-panel-title">未来 7 天降水预报</h2><span v-if="ready" class="selected-day">{{ selectedLabel }}</span></div>
    ...
    <template v-if="ready">
      ...
      <div class="controls-legend-row">
        ...
        <div class="legend" aria-label="当日累计降雨量图例">
          <span class="legend-title" aria-hidden="true">图例</span>
...
const selectedLabel = computed(() => { const index = props.selectedDay; const date = parseDay(days.value[index] ?? ''); return Number.isNaN(date.getTime()) ? '' : `${date.getMonth() + 1}/${date.getDate()} ${['周日', '周一', '周二', '周三', '周四', '周五', '周六'][date.getDay()]}` })
...
.precip-panel {
  ...
  border: 1px solid rgba(37, 99, 235, 0.55); /* 专题模式身份：action-blue 边框（Design.md：蓝色标识专题） */
  border-radius: 10px;
...
.selected-day { font-size: 10px; color: #1d4ed8; font-weight: 600; white-space: nowrap; }
...
.opacity-control { display: flex; align-items: center; gap: 6px; margin-left: 2px; }
.opacity-control label { white-space: nowrap; color: #64748b; font-size: 11px; }
.opacity-control input[type='range'] { width: 88px; accent-color: #60a5fa; height: 14px; margin: 0; }
.opacity-value { min-width: 28px; text-align: right; color: #64748b; font-size: 10px; font-variant-numeric: tabular-nums; }
/* 图例：独立说明块（浅灰底 + 小标题，与时间轴数据区明确解耦） */
.legend {
  flex: none;
  width: 238px;
  display: flex;
...
.legend-title { font-size: 8.5px; color: #94a3b8; line-height: 1; }
.legend-bar { display: flex; height: 5px; border-radius: 2px; overflow: hidden; }
.legend-swatch { flex: 1; }
.legend-swatch + .legend-swatch { border-left: 1px solid rgba(248, 250, 252, 0.9); }
.legend-labels { display: flex; }
.legend-labels span { flex: 1; text-align: center; font-size: 8px; color: #475569; line-height: 1; white-space: nowrap; }
