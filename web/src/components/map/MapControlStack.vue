<template>
  <div ref="controlStackRef" class="ctrl-stack">
    <button
      type="button" class="icon-btn weather-btn" disabled
      title="查看天气为预留功能，本期暂不实现" aria-label="查看天气为预留功能，本期暂不实现"
    >
      ...
      <span class="icon-tip" role="tooltip">查看天气为预留功能，本期暂不实现</span>
    </button>
...
const controlStackRef=ref<HTMLElement|null>(null), parcelToolButtonRef=ref<HTMLButtonElement|null>(null)
const firstParcelActionRef=ref<HTMLButtonElement|null>(null)
const parcelMenuOpen=ref(false)
const typhoonTip=computed(()=>props.disasterActive?'台风查看模式已开启':props.disasterEntryDisabled?'请先保存或取消当前未完成操作':'查看台风')
...
function closeOnOutside(event:PointerEvent){if(parcelMenuOpen.value&&!controlStackRef.value?.contains(event.target as Node))closeMenus()}
function onKeydown(event:KeyboardEvent){if(event.key!=='Escape'||!parcelMenuOpen.value)return;event.preventDefault();event.stopImmediatePropagation();closeMenus();void nextTick(()=>parcelToolButtonRef.value?.focus())}
watch(()=>[props.parcelToolsVisible,props.parcelToolsDisabled] as const,()=>{if(!props.parcelToolsVisible||props.parcelToolsDisabled)parcelMenuOpen.value=false})
