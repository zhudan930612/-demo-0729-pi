<template>
  <div ref="controlStackRef" class="ctrl-stack">
    <button
      ref="weatherButtonRef" type="button" class="icon-btn weather-btn" :class="{ active: weatherActive }"
      :disabled="weatherEntryDisabled || weatherActive" :title="weatherTip" :aria-label="weatherTip" @click="emit('open-weather')"
    >
...
const emit = defineEmits<{ 'switch-basemap':[type:'img'|'vec']; 'toggle-rs':[]; 'toggle-parcels':[]; 'start-manual':[]; 'start-filter':[]; 'open-typhoon':[]; 'open-weather':[]; 'zoom-in':[]; 'zoom-out':[] }>()
