...
const parcelMenuOpen=ref(false),weatherMenuOpen=ref(false),layerMenuOpen=ref(false)
...
function toggleParcelMenu(){if(props.parcelToolsDisabled)return;weatherMenuOpen.value=false;layerMenuOpen.value=false;parcelMenuOpen.value=!parcelMenuOpen.value;if(parcelMenuOpen.value)void nextTick(()=>firstParcelActionRef.value?.focus())}
function toggleWeatherMenu(){if(props.weatherEntryDisabled)return;parcelMenuOpen.value=false;layerMenuOpen.value=false;weatherMenuOpen.value=!weatherMenuOpen.value;if(weatherMenuOpen.value)void nextTick(()=>firstWeatherActionRef.value?.focus())}
function toggleLayerMenu(){parcelMenuOpen.value=false;weatherMenuOpen.value=false;layerMenuOpen.value=!layerMenuOpen.value}
...
function closeOnOutside(event:PointerEvent){if((parcelMenuOpen.value||weatherMenuOpen.value||layerMenuOpen.value)&&!controlStackRef.value?.contains(event.target as Node))closeMenus()}
function onKeydown(event:KeyboardEvent){if(event.key!=='Escape'||(!parcelMenuOpen.value&&!weatherMenuOpen.value&&!layerMenuOpen.value))return;event.preventDefault();event.stopImmediatePropagation();closeMenus()}
