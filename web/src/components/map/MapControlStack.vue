  ...
  <div ref="controlStackRef" class="ctrl-stack">
    <div class="tool-entry">
      <button
        ...
        :title="disasterTip" :aria-label="disasterTip" aria-haspopup="true" :aria-expanded="disasterMenuOpen" aria-controls="disaster-tool-menu"
      ...
      <Transition name="tool-menu">
        <div v-if="disasterMenuOpen" id="disaster-tool-menu" class="tool-menu" aria-label="选择灾害风险查看项">
          <button ref="firstDisasterActionRef" type="button" class="menu-action" @click="chooseTyphoon"><span class="menu-icon" aria-hidden="true">台</span><span>查看台风</span></button>
          <button type="button" class="menu-action" disabled aria-describedby="weather-unavailable" title="查看天气为预留功能，本期暂不实现"><span class="menu-icon" aria-hidden="true">云</span><span>查看天气</span><small>预留</small></button>
          <p id="weather-unavailable" class="menu-note">查看天气为预留功能，本期暂不实现</p>
        </div>
      </Transition>
    </div>
