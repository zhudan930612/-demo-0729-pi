...
describe('weather store',()=>{beforeEach(()=>setActivePinia(createPinia()))
 it('地址降级不算部分失败，天气模块 stale refreshError 才算',()=>{const s=useWeatherStore();s.bundle=bundle();expect(s.hasPartialFailure).toBe(false);(s.bundle.current as unknown as { stale?: boolean; refreshError?: { code: string; message: string } }).stale=true;(s.bundle.current as unknown as { stale?: boolean; refreshError?: { code: string; message: string } }).refreshError={code:'x',message:'失败'};expect(s.hasPartialFailure).toBe(true)})
