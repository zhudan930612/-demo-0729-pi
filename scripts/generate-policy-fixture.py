...
def generate(code: str) -> None:
    ...
    # 标注区域模式：大户身份以确认文件 spatialReview（insuredModePreview=single_insured）为准，
    # 区域外单块（含面积 >50 亩的团单块，如龙江村地块 1424）一律一块一户进团单
    ...
    for confirmed_party_id, group in sorted(confirmed_groups.items(), key=lambda item: item[0]):
        ...
        if region_mode:
            # 标注区域模式：区域 party 单独出单一型保单；其余单块 party 一块一户进团单
            mode = "single_insured" if confirmed_party_id in single_parties else "insured_roster"
        else:
            # 聚类模式：50 亩分类规则按被保险人汇总面积（非地块数）：>50.00 亩单独出单一型保单（大户），
