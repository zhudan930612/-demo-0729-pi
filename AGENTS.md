...
> **样式/文案例外（用户确认）**：纯样式/文案改动（不涉逻辑、不改 e2e 依赖的类名/选择器）**无需任何验证**，直接改 + `git add` + 提交；仅当用户明确要求「去查勘/验证」，或改动涉及逻辑/可能编译不过时才补 build + 定向测试。勿为样式改动循环跑 build/test/e2e/截图。

### 快速层（必做，改动涉逻辑/编译时；纯样式/文案除外）
