...
const spatial = { findAlarmNode(code) { return code === '330100' ? { level: 'city', representativePoint: [120, 30] } : code === '330000' ? { level: 'province', representativePoint: [120, 29] } : null } }
...
test('national alarm spatial index loads only map-relevant Zhejiang levels', () => {
  ...
  assert.equal(index.findAlarmNode('330000').level, 'province'); assert.equal(index.findAlarmNode('330100').level, 'city'); assert.equal(index.findAlarmNode('330101').level, 'county'); assert.equal(index.findAlarmNode('330101001000'), null)
})
...
test('normalization emits Zhejiang whitelist only and has no source path', () => { const result = normalizeNationalAlarms(rows, spatial); assert.equal(result.items.length, 1); assert.equal(result.items[0].provinceCode, '33'); assert.equal(result.items[0].mappableInZhejiang, true); assert.equal('sourcePath' in result.items[0], false); assert.deepEqual(result.items[0].mapLocation.point, [120, 30]) })
